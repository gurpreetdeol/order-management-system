...
async def get_category_by_id(category_id: int, database) -> models.Category:
...
async def delete_category_by_id(category_id: int, database):
    database.query(models.Category).filter(models.Category.id == category_id).delete(synchronize_session=False)
    database.commit()
